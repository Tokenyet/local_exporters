...
bundle_name="twitch-local-exporter-$tag-macos-$platform_arch"
bundle_root="$release_dir/$bundle_name"
unpacked_output="${output_dir%/}/unpacked/twitch-local-exporter"
extension_zip="$release_dir/twitch-local-exporter-extension-$tag-macos-$platform_arch.zip"
bundle_zip="$release_dir/$bundle_name.zip"
host_source="$ROOT/native-host/dist/twitch-local-exporter-host"
host_asset="$release_dir/twitch-local-exporter-host-$tag-macos-$platform_arch"
...
mkdir -p "$bundle_root/native-host/twitch_local_exporter"
for file in "$ROOT"/native-host/twitch_local_exporter/*.py; do
    cp "$file" "$bundle_root/native-host/twitch_local_exporter/"
done
if [[ -f "$host_source" ]]; then
    cp "$host_source" "$bundle_root/native-host/twitch-local-exporter-host"
    chmod +x "$bundle_root/native-host/twitch-local-exporter-host"
...
Path(sys.argv[4]).write_text(f"# Twitch Local Exporter v{sys.argv[3]} (macOS)\n\n{notes}\n\n## Install From This Release\n\n{install}", encoding="utf-8")
PY
