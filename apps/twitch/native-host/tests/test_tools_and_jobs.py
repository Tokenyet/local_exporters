...
from twitch_local_exporter.config import product_tools_dir, toolchain_mode, toolchain_root, tools_dir
from twitch_local_exporter.tools import normalize_model_name, require_tools, whisper_backend, yt_dlp_js_runtime_args
        ...
        try:
            with tempfile.TemporaryDirectory() as tmp:
                ...
                settings_path.write_text(json.dumps({
                    "products": {
                        "twitch": {"mode": "shared", "root": str(root)}
                    }
                }), encoding="utf-8")
                ...
                self.assertEqual(product_tools_dir(), root / "products" / "twitch")
        finally:
    ...
    def test_whisper_backend_prefers_cuda_binary_when_cuda_is_available(self):
        ...
        try:
            with tempfile.TemporaryDirectory() as tmp:
                ...
                settings_path.parent.mkdir(parents=True)
                settings_path.write_text(json.dumps({
                    "products": {"twitch": {"mode": "shared", "root": str(root)}}
                }), encoding="utf-8")
                ...
                with patch("twitch_local_exporter.tools.cuda_available", return_value=True):
