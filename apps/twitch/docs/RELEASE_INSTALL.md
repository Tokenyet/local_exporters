Download the matching platform bundle: `twitch-local-exporter-vX.Y.Z-windows.zip` on Windows or `twitch-local-exporter-vX.Y.Z-macos-arm64.zip` / `twitch-local-exporter-vX.Y.Z-macos-x64.zip` on macOS. Extract it, then load this folder in your browser extension page:
