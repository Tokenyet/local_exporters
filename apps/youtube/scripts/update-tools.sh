...
temp_dir="$(mktemp -d "${TMPDIR:-/tmp}/youtube-local-exporter-tools.XXXXXX")"
cleanup() {
    rm -rf "$temp_dir"
}
trap cleanup EXIT
...
"$python_bin" - "$manifest_dir/$PRODUCT_KEY.json" "$PRODUCT_KEY" "$mode_lower" "$resolved_toolchain_root" "$machine" "$whisper_model" "$whisper_acceleration" "$yt_version" "$deno_version" "$ffmpeg_version" "$whisper_version" <<'PY'
import json
import sys
