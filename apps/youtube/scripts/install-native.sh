...
DEFAULT_APP_DIR="$HOME/Library/Application Support/YouTubeLocalExporter"
...
launcher_path="$app_dir/youtube-local-exporter-host"
built_host="$ROOT/native-host/dist/youtube-local-exporter-host"
installed_host="$app_dir/youtube-local-exporter-host"
...
if [[ -f "$built_host" ]]; then
    ...
    echo "Installing the native host launcher..."
...
exec "$python_bin" "\$SCRIPT_DIR/native-host/youtube_local_exporter_host.py" "\$@"
EOF
...
echo "Installed YouTube native host in $app_dir"
